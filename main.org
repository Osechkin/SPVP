...
volatile uint32_t data_samples_len;							// количество объектов DataSample и соответствующих им эхо, записанных в data_samples
volatile uint32_t current_data_sample;						// текущий элемент в data_samples (счетчик готовых, но еще необработанных данных)
DataSample *data_samples[4*UPP_DATA_COUNT];					// массив объектов, содержащий указатели на полученные из АЦП данные (в samples_buffer) и описание к этим данным (см. DataSample).
...
void executeProcPack(DataSample *ds, Data_Proc *proc, int index);
...
void main(void)
{
	...
	// Data containers ------------------------------------------------------------------
	//initDataHeap();
	// буфер обработанных и "полу-обработанных" данных
	...
	// буфер первичных данных (данные с АЦП и данные других устройств)
	/*
	samples_buffer = (uint8_t**) calloc(UPP_DATA_COUNT, sizeof(uint8_t*));
	for (i = 0; i < UPP_DATA_COUNT; i++)
	{
		uint8_t *upp_arr = (uint8_t*) calloc(DATA_MAX_LEN, sizeof(uint16_t));
		samples_buffer[i] = upp_arr;
	}
	*/
	...
	while (app_finish == 0)
	{
		...
		if (seq_completed > 0)
		{
			...
			// process data
			int j;
			for (j = 0; j < new_data_counter; j++)
			{
				DataSample *ds = data_samples[ds_proc_data_index];	// забрать свежие данные
				uint8_t _proc_index = ds->proc_id;					// определить номер пакета команд обработки поступивших данных

				if (_proc_index <= MAX_PROCS && _proc_index > 0)
				{
					move_ToFirstDataProcCmd(_proc_index - 1, instr_prg);
					executeProcPack(ds, instr_prg, _proc_index - 1);
					ds->tag = 1;		// обработка завершена
				}
				else
				{
					printf("Bad Processing package number: %d\n", _proc_index);
				}

				ds_proc_data_index++;
				if (ds_proc_data_index >= UPP_DATA_COUNT)
				{
		...
		if (tool_state == READY) // прибор готов к приему/передаче данных по кабелю (получен сигнал GP0[3] "up")
		{
			...
			//else if (temperature_mode == 2) if (UART_telemetric_counter % TELEMETRIC_DATA_LEN2 == 0 && UART_telemetric_counter > 0) tele_flag = 1;
...
void executeProcPack(DataSample *ds, Data_Proc *proc, int index)
{
	...
	while (next_DataProcCmd(index, proc, instr) == True)
	{
		...
		case INS_EMUL_FID:		emulate_FIDData(upp_data_ptr, processing_params); break;
		case INS_EMUL_SE:		emulate_EchoData(upp_data_ptr, processing_params); break;
		case INS_EMUL_FID_NS:	emulate_FIDNoiseData(instr, upp_data_ptr, processing_params); break;
		case INS_EMUL_SE_NS:	emulate_EchoNoiseData(instr, upp_data_ptr, processing_params); break;
		case INS_OPER_FID:		cast_UPPDataToFID(upp_data_ptr, processing_params->points_count, ptr_data_org); break;
		case INS_OPER_SE:		cast_UPPDataToSE(upp_data_ptr, processing_params->points_count,	ptr_data_org); break;
		case INS_OPER_FID_D:	cast_UPPDataToFID2(upp_data_ptr, processing_params, ptr_data_org); break;
		case INS_OPER_SE_D:		cast_UPPDataToSE2(upp_data_ptr, processing_params, ptr_data_org); break;
		...
		case INS_NOISE_UPP_PRE1:	noiseUppDataPreprocessing1(upp_data_ptr, bank, data_heap, &data_heap_len[0], processing_params, instr); break;
		case INS_SGN_UPP_PRE1:		signalUppDataPreprocessing1(upp_data_ptr, bank, data_heap, processing_params, instr); break;
		//case INS_NS_SGN_UPP_PRE3:	signal_noise_UppDataPreprocessing3(upp_data_ptr, bank, data_heap, &data_heap_len[0], processing_params, instr); break;
		...
		case INS_NOISE_PROC1:		noiseProcessing1(upp_data_ptr, bank, rad, instr, processing_params, output_data); break;
	...
	if (gpioRegs->BANK[0].INTSTAT & CSL_GPIO_INTSTAT_STAT1_MASK)
	{
		...
		else if (pin1_state == 1)
		{
			...
			case NMR_TOOL:
			{
				...
				if (current_data_sample < UPP_DATA_COUNT - 1) current_data_sample++;
				else current_data_sample = 0;
			...
			case GAMMA_TOOL:
			{
				...
				ds_new_data_enabled = 1;
				if (device_id == DUMMY_TOOL) nmr_data_ready = 1;	// можно приступить к обработке данных NMR
