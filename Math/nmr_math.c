 ...
 *  - длина эмулированного массива proc_params->points_count (числа в массиве - целые двухбайтные)
 */
void emulate_NoiseData(uint8_t *dst, int data_len)
{
	...
	{
		int16_t tab_noise = 0;
		if (i+shift < NMR_DATA_LEN) tab_noise = noise_ui16[i+shift] - 2048;		// каждая выборка шума берется как фрагмент заранее приготовленной большой выборки шума, начиная со случайного места shift
		else tab_noise = noise_ui16[i] - 2048;
...
void signal_noise_UppDataPreprocessing(DataSample *ds, float **data_bank, float **hx_bank, int *hx_len, Processing_Params *proc_params, Data_Cmd *instr)
{
	if (instr->count != 6) return;
	...
	float a = (float)instr->params[5]/1000.0;	// параметр, на который необходимо домножать первое эхо сигнала
	if (a == 0) a = 1;
	if (ds->echo_number != NN + 1) a = 1; // домножается на a только первое эхо сигнала

	int src_len = ds->data_len;
	if (src_len == 0) return;

	//t_stop = clock();
	//t[0] = ((t_stop - t_start) - t_overhead);
	//t_start = clock();
	...
	//t_stop = clock();
	//t[1] = ((t_stop - t_start) - t_overhead);
	//t_start = clock();

	int i;
	int current_echo = ds->echo_number;
	...
	//t_stop = clock();
	//t[4] = ((t_stop - t_start) - t_overhead);
	//printf(" NMR data processing time:\n Initialization: \t%d ms\n Zeroization: \t%d ms\n Emulation: \t%d ms\n Transformation: \t\%d ms\n Data Saving: \t\%d ms\n\n", t[0],t[1],t[2],t[3],t[4]);
}
...
/* Main function for NMR data preprocessing and processing
 * Input parameters:
 * data_samples -		массив структур типа DataSample, т.е. хранилище первичных эхо, полученных из UPP. Содержит также полную информацию от каждом эхо (см. описание DataSample)
 * ds_count 	- 		количество эхо в массиве data_samples
 *
 */
void NMR_data_Processing(DataSample *data_samples[], int ds_count, int rad, float **data_bank, Processing_Params *proc_params, Data_Cmd *instr, OutBuffer *out_buff)
{
	if (instr->count != 9) return;

	int data_src = (int)instr->params[0];			// источник данных ибо отношение сигнал/шум (данные с АЦП = 0, если эмулировать данные, то >0, причем число = отношению сигнал/шум)
	...
	int NS = (int)instr->params[3];					// количество окон сигнала
	int Nacc = (int)instr->params[4];				// число накоплений
	int out_time_data = (int)instr->params[5];		// выводить/не выводить накопленные во временной области данные АЦП: если не выводить, то = 0, если вывести, то = номеру эхо
	int out_spec_data = (int)instr->params[6];		// выводить/не выводить спектр мощности сигнала после: мощностной спектр фурье, наложение окна в спектральной области. Если =0, то не выводить, иначе = номеру эхо, от которого бедется спектр
	int group_index = (int)instr->params[7];		// групповой индекс
	uint8_t data_code = (uint8_t)instr->params[8];	// код данных
	...
	if (ds_count < NN) return;						// измерено слишком мало выборок шума (сбой при измерениях)
	//if (ds_count < NN+NS) NS = ds_count - NN;		// измерено количество выборок шума и сигнала меньшее, чем ожидалось (сбой при измерениях). Работаем с теми сигналами, которые удалось измерить
	//if (ds_count > NN+NS) ds_count = NS + NN;		// в структуре data_samples слишком много выборок сигнала и шума (возможно "лишние данные" - это данные каких-то других измерений. Отсечь лишнее
	...
	int ns_counter = 0;								// счетчик выборок шума
	int sg_counter = 0;								// счетчик выборок сигнала
	if (data_src > 0)
	{
		for (k = 0; k < ds_count; k++)
		{
			DataSample *ds = data_samples[k];
			uint16_t src_number = ds->echo_number;
			uint8_t *upp_data = ds->data_ptr;
			int src_len = ds->data_len;
	...
	float *data_store = data_bank[1];				// array in L2-stack data1. Используется для накопления эхо шума или сигнала
	float *data_integrals = data_bank[2];			// array in L2-stack data2. Используется для накопления интегралов эхо
	float *ptr_w = data_bank[9];

	float *temp_data = data_bank[8];
	//memset(temp_data, 0x00, NMR_DATA_LEN*sizeof(float));
	memset(data_integrals, 0x00, NMR_DATA_LEN*sizeof(float));
	memset(data_store, 0x00, NMR_DATA_LEN*sizeof(float));

	// обработка выборок шума и накопление интеграла от спектра выборок шума
	int src_pre = 0;
	int dst_pre = 0;								// начальная позиция данных центрированного эхо в буфере типа float (перед этой позицией - нули)
	int dst_post = DATA_MAX_LEN;					// конечная позиция данных центрированного эхо в буфере типа float (после этой позиции - нули)

	float SNs = 0;									// накопленный интеграл под спектром мощности эхо
	int src_len = DATA_MAX_LEN;
	int kpmg_len = NN + NS;
	for (k = 1; k <= kpmg_len; k++)
	{
		SNs = 0;
		int acc_number = 0;							// реальное количество накоплений эхо шума с номером k
		for (m = 0; m < ds_count; m++)
		{
			DataSample *ds = data_samples[m];
			uint16_t src_number = ds->echo_number;
			uint8_t *upp_data = ds->data_ptr;
			src_len = ds->data_len;
			...
			if (src_number == k)
			{
				...
				S /= src_len;
				memset(data, 0x0, dst_pre*sizeof(float));
				memset(data+dst_post, 0x0, dst_pre*sizeof(float));
				// --------------------------------------------

				// пересчет точек эхо от отсчетов АЦП к mV ----
				index = dst_pre;
				for (i = 0; i < src_len; i++)
				{
					float x = data[index];
					data[index++] = a*(x - S)*ADC_to_mV;
				}
				// --------------------------------------------

				// Накопление эхо (шума) ЯМР ------------------
				index = dst_pre;
				float invAcc = 1.0/acc_number;
				for (i = 0; i < src_len; i++)
				{
					data_store[i+index] = (data_store[i+index]*(acc_number-1) + data[i+index])*invAcc;
				}
				memset(data_store, 0x0, dst_pre*sizeof(float));
				memset(data_store+dst_post, 0x0, dst_pre*sizeof(float));
				// --------------------------------------------
			}
		}

		// наложить оконную функцию на накопленное эхо во временнОй области
		if (acc_number > 0)
		{
			// ----- оконная функция во временной области -----
			applyWinFunc(data_store, data_store, DATA_MAX_LEN, proc_params, TIME_DOMAIN_DATA);
			// ------------------------------------------------

			// - копирование в выходной буфер временнЫх данных накопленного шума
			if (out_time_data > 0 && k == out_time_data && acc_number == 1)
			{
				float *dst = out_buff->out_data;
				int dst_pos = out_buff->full_size;
				int data_cnt = out_buff->outdata_counter;
				...
				for (i = 0; i < src_len; i++)
				{
					dst[dst_pos+i] = data_store[pre_pos+i];
				}
				out_buff->outdata_len[data_cnt] = src_len;
				out_buff->full_size += src_len;
				out_buff->data_id[data_cnt] = DT_NS_SE;
				out_buff->channel_id[data_cnt] = proc_params->channel_id;
				out_buff->outdata_counter++;
			}
			// ------------------------------------------------

			// --------- быстрое преобразование Фурье ---------
			memcpy(temp_data - PAD, data_store - PAD, UPP_DATA_SIZE); 	// дублирование данных src, т.к. они разрушаются при выполнении функции DSPF_sp_fftSPxSP(...)
			DSPF_sp_fftSPxSP(CMPLX_DATA_MAX_LEN, temp_data, ptr_w, data_store, brev, rad, 0, CMPLX_DATA_MAX_LEN);
			// ------------------------------------------------
			...
			for (i = 0; i < DATA_MAX_LEN; i += 2)
			{
				data_store[index++] = data_store[i]*data_store[i] + data_store[i+1]*data_store[i+1];	// power spectrum
			}
			// ------------------------------------------------

			// ----- оконная функция в частотной области ------
			applyWinFunc(data_store, data_store, DATA_MAX_LEN/2, proc_params, FREQ_DATA);
			// ------------------------------------------------

			// помещение в выходной буфер частотного спектра мощности шума
			if (out_spec_data > 0  && k == out_spec_data && acc_number == 1)
			{
				float *dst = out_buff->out_data;
				int dst_pos = out_buff->full_size;
				int data_cnt = out_buff->outdata_counter;
				...
				{
					*(dst+dst_pos+i) = Q_rsqrt(data_store[i]);
				}
			...
			for (i = 0; i < DATA_MAX_LEN/2; i++)
			{
				SNs += Q_rsqrt(data_store[i]);
			}
			//SNs /= 512.0;
		}
		data_integrals[k-1] = SNs;
	}

	// --- Усреднение интегралов под спектрами выборок шума ---
	SNs = 0;
	if (ns_counter > 0)
	{
	...
	// --- Накопление интеграла эхо в буфере сумирования ------
	for (k = 0; k < kpmg_len-NN; k++)
	{
		data_integrals[k+NN] = (data_integrals[k+NN] - SNs)/512.0;
	}
	...
	memcpy(dst+dst_pos, &data_integrals[NN], (kpmg_len-NN)*sizeof(float));
	out_buff->outdata_len[data_cnt] = kpmg_len-NN;
	out_buff->full_size += (kpmg_len-NN);
	out_buff->data_id[data_cnt] = data_code;
	out_buff->channel_id[data_cnt] = proc_params->channel_id;
	out_buff->group_index[data_cnt] = group_index;
	out_buff->outdata_counter++;
