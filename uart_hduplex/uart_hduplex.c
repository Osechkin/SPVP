...
void init_GPIO()
{
	...
	CSL_FINS(gpioRegs->BANK[0].DIR, GPIO_DIR_DIR13, CSL_GPIO_DIR_DIR_OUT);
	CSL_FINS(gpioRegs->BANK[0].DIR, GPIO_DIR_DIR14, CSL_GPIO_DIR_DIR_OUT);
	CSL_FINS(gpioRegs->BANK[0].DIR, GPIO_DIR_DIR15, CSL_GPIO_DIR_DIR_OUT);
...
{
	unsigned int pin;
	//pin = gpioRegs->BANK[0].IN_DATA;		// to read pins 7-12
	pin = proger_read_gpio();				// to read memory segment which is reflection of the pins 7-12
	return ( pin );
}
