...
#define DATA_HEAP_COUNT		(4096)			// количество массивов в куче для долгосрочного хранения данных
#define UPP_DATA_COUNT		(8192)			// количество массивов для временного хранения данных АЦП (до их последующей обработки)
...
#define TELEMETRIC_DATA_LEN 9				// количество байт в одном измерении - для KMRK, NMKT,...
#define TELEMETRIC_UART_BUF_LEN 9+9+9		// количество измеренных параметров ( три сопротивления (x3 bytes), три тока по три значения (x2 bytes), три напряжения по три значения (x2 bytes) ) - для KMRK, NMKT
#define TELEMETRIC_DATA_LEN2 2				// количество байт измеренной температуры (2 байта) - для SDSP
#define TELEMETRIC_UART_BUF_LEN2 24			// количество байт измеренных температур (до 8 датчиков 2 байта) и напряжений (4 датчика напряжения по 2 байта) - для SDSP
...
#define NMR_TOOL            0xFE        	// NMR logging tool (адрес устройства ЯМР на плате MityDSP )
...
#define FPGA_PRG			0x2D			// программа для интервального программатора (2^18 of Galois)
// Format of command:
// Command (1 byte)
