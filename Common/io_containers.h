...
{
	uint8_t		*data_ptr;							// указатель на источник данных (обычно данные ј÷ѕ - только дл€ яћ–)
	int32_t		data_len;							// длина данных (только дл€ яћ–)
	int32_t		echo_number;						// номер эхо (по счетчику hard_echo_counter - только дл€ яћ–)
	uint8_t		tool_id;							// id устройства
	uint8_t		proc_id;							// номер пакета обработки данных
	uint8_t		channel_id;							// номер канала данных
	uint8_t		tag;								// дополнительное поле дл€ отладочных целей или прочих целей пользовател€
} DataSample;
