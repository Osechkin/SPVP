...
int findMsgPacket(BUFFER8 *_buff, UART_Message *_uart_msg, GF_Data *_gf_data)
{
	MsgHeader *_msg_header = _uart_msg->msg_header;
	...
	MsgPacket *pack = (MsgPacket*)malloc(sizeof(MsgPacket));
	//MsgPacket *pack = _uart_msg->msg_packs[pack_number];
	initMsgPacket(_msg_header, pack, pack_number);				// до расшифровки пакета не знаем его номер - поэтому присваиваем номер 0 !
	uint8_t *data_arr = &pack->data[0];
	uint8_t *data_cnt = &pack->data_len;
	...
	pack_number = pack->data[2];
	if (pack_number > 0 && pack_number <= pack_count)
	{
		pack->packet_number = pack_number;
		_uart_msg->msg_packs[pack_number-1] = pack;
		_uart_msg->pack_cnt = pack_number;			// механизм подсчета пакетов - calculatePackets(...)
	}
	else
	{
		free(pack);
...
int checkMsgPacket(MsgPacket *_pack, UART_Message *_uart_msg)
{
	MsgHeader *_msg_header = _uart_msg->msg_header;

	MsgPacket *pack = _pack;
	uint8_t type = pack->data[0];
	if (type != MTYPE_PACKET) return E_MSG_TYPE;
...
int calculatePackets(UART_Message *_uart_msg)
{
	int pack_count = 0;
	int i;
	for (i = 0; i < _uart_msg->pack_cnt; i++)
	{
		MsgPacket *pack = _uart_msg->msg_packs[i];
		if (pack->packet_number > 0) pack_count++;
	}
